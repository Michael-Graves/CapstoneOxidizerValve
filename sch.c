...
#if SYSTICK_ENABLE == 1
#define SYSTICK_PERIOD 8000    //80,00 = 0.1 millisecond @ 80MHz
...
volatile struct TaskStructure TaskList[MAX_NUMBER_OF_TASKS];

uint8_t numOfTaskTimers = 0;
volatile struct TaskTimerStructure TaskTimerList[MAX_NUMBER_OF_TASKS];
    ...
    for (currentTask = 0; currentTask < numOfTasks; currentTask++) {
        ...
        case TASK_STATE_IDLE:
            ...
            case TASK_CONDITION_TYPE_BOOL: case TASK_CONDITION_TYPE_TIMER:
                ...
                taskCondition = *((_Bool*)TaskList[currentTask].TaskCondition);
                if(taskCondition) {
